...
and g' oc = function
	...
	| Tail, IfFEq(x, y, e1, e2) ->
		g'_tail_if oc x y e2 e1 "fbne" "fbeq"
	...
	| NonTail(z), IfFEq(x, y, e1, e2) ->
		g'_non_tail_if oc (NonTail(z)) x y e2 e1 "fbne" "fbeq"
...
let f oc (Prog(fundefs, e)) =
	...
	(* 0x1fffff *)
	Output.add_stmt (Output.Mvhi (reg_sp, 0x1f));
	Output.add_stmt (Output.Mvlo (reg_sp, 0xffff));
